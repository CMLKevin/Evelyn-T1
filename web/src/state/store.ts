...
export interface CodeFile {
  ...
  // Progress indicators
  status?: 'idle' | 'generating' | 'updating' | 'complete' | 'error';
  linesAdded?: number;
  linesRemoved?: number;
...
interface CodeSubtask {
  ...
  createdAt: string;
  completedAt?: string;
  // Phase 14: Enhanced progress tracking
  generationPhase?: 'idle' | 'planning' | 'generating' | 'applying' | 'complete' | 'failed';
...
interface CodeCanvas {
  ...
  files: CodeFile[];
  tasks: CodeTask[];
  collaborations: CodeCollaboration[];
}

interface DiffApprovalRequest {
  subtaskId: number;
  filename: string;
  filepath: string;
  diff: {
...
// Phase 14: Operation metrics tracking
interface OperationMetrics {
  generationSpeed: number; // lines per minute
  qualityScore: number; // 0-100
  confidenceScore: number; // 0-1
}

// Phase 14: Error recovery tracking
interface ErrorRecoveryAttempts {
  attemptCount: number;
  lastAttemptTime: string;
  successRate: number; // 0-1
}

// Phase 14: Active operation tracking
interface ActiveOperation {
  id: string;
  type: 'task' | 'subtask' | 'file' | 'suggestion';
  targetId: number;
  phase: 'planning' | 'generating' | 'applying';
  progress: number; // 0-100
  startedAt: string;
  estimatedCompletion?: string;
}

// Phase 14: Operation history for timeline
...
// Phase 14: Generation statistics
...
interface CanvasState {
  activeCanvas: CodeCanvas | null;
  canvasList: CodeCanvas[];
  ...
  pendingDiffApproval: DiffApprovalRequest | null;
  // File status tracking
  fileStatuses: Map<number, {
    status: 'idle' | 'generating' | 'updating' | 'complete' | 'error';
    linesAdded?: number;
    linesRemoved?: number;
    lastModifiedAt: string;
  }>;
  // Phase 14: Enhanced state management
  operationMetrics: OperationMetrics;
  errorRecoveryAttempts: Map<number, ErrorRecoveryAttempts>; // keyed by subtask ID
...
interface Store {
  ...
  // Code Canvas actions
  setActiveCanvas: (canvas: CodeCanvas | null) => void;
  setCanvasList: (canvases: CodeCanvas[]) => void;
  setSelectedFile: (file: CodeFile | null) => void;
  updateCanvasFile: (fileId: number, content: string) => void;
  setGeneratingStatus: (isGenerating: boolean, progress?: { total: number; completed: number; currentTask: string } | null) => void;
  loadCanvasList: () => Promise<void>;
  loadCanvas: (canvasId: number) => Promise<void>;
  createCanvasFromSession: (canvasData: any) => void;
  updateFileContent: (canvasId: number, fileId: number, content: string) => Promise<void>;
  renameCanvas: (canvasId: number, title: string) => Promise<void>;
  
  // Diff approval workflow actions
  setDiffApprovalRequest: (request: DiffApprovalRequest) => void;
  clearDiffApprovalRequest: () => void;
  
  // File status tracking actions
  updateFileStatus: (fileId: number, status: {
    status: 'idle' | 'generating' | 'updating' | 'complete' | 'error';
    linesAdded?: number;
    linesRemoved?: number;
  }) => void;
  clearFileStatus: (fileId: number) => void;
  clearAllFileStatuses: () => void;
  
  // Tab management actions
  openFileInTab: (file: CodeFile) => void;
  closeTab: (fileId: number) => void;
  setOpenTabs: (tabs: CodeFile[]) => void;
...
export const useStore = create<Store>((set, get) => ({
  ...
  canvasState: {
    ...
    pendingDiffApproval: null,
    fileStatuses: new Map(),
    // Phase 14: Initialize new state
    operationMetrics: {
      generationSpeed: 0,
      qualityScore: 0,
      confidenceScore: 0
    },
    errorRecoveryAttempts: new Map(),
    activeOperations: [],
    operationHistory: [],
  ...
  setActiveCanvas: (canvas) => set((state) => ({
    canvasState: { ...state.canvasState, activeCanvas: canvas }
  })),
  
  setCanvasList: (canvases) => set((state) => ({
    canvasState: { ...state.canvasState, canvasList: canvases }
  })),
  
  setSelectedFile: (file) => set((state) => ({
    canvasState: { ...state.canvasState, selectedFile: file }
  })),
  
  updateCanvasFile: (fileId, content) => set((state) => {
    if (!state.canvasState.activeCanvas) return state;
    
    const updatedFiles = state.canvasState.activeCanvas.files.map(f =>
    ...
    return {
      canvasState: {
        ...state.canvasState,
        activeCanvas: {
          ...state.canvasState.activeCanvas,
          files: updatedFiles
        }
      }
    };
  ...
  setGeneratingStatus: (isGenerating, progress = null) => set((state) => ({
    canvasState: {
      ...state.canvasState,
      isGenerating,
  ...
  loadCanvasList: async () => {
    try {
      console.log('[Store] Loading canvas list...');
      const response = await fetch('http://localhost:3001/api/canvas?limit=50');
      if (response.ok) {
        const canvases = await response.json();
        console.log(`[Store] Loaded ${canvases.length} canvases`);
        set((state) => ({
          canvasState: { ...state.canvasState, canvasList: canvases }
        }));
      }
    } catch (error) {
      console.error('[Store] Failed to load canvases:', error);
    }
  },
  
  loadCanvas: async (canvasId) => {
    try {
      console.log(`[Store] Loading canvas ${canvasId}...`);
      const response = await fetch(`http://localhost:3001/api/canvas/${canvasId}`);
      if (response.ok) {
        const canvas = await response.json();
        console.log(`[Store] Loaded canvas: ${canvas.title}`);
        
        // Restore open tabs and selected file from localStorage
        let openTabs: CodeFile[] = [];
        let selectedFile: CodeFile | null = null;
        
        try {
          const savedTabIds = localStorage.getItem(`canvas-${canvasId}-open-tabs`);
          const savedSelectedFileId = localStorage.getItem(`canvas-${canvasId}-selected-file`);
          
          if (savedTabIds) {
            const tabIds = JSON.parse(savedTabIds);
            openTabs = tabIds
              .map((id: number) => canvas.files.find((f: CodeFile) => f.id === id))
              .filter((f: CodeFile | undefined) => f !== undefined) as CodeFile[];
          }
          
          if (savedSelectedFileId) {
            selectedFile = canvas.files.find((f: CodeFile) => f.id === parseInt(savedSelectedFileId)) || null;
          }
          
          // Fallback if no valid state was restored
          if (!selectedFile && canvas.files.length > 0) {
            selectedFile = canvas.files[0];
            openTabs = [canvas.files[0]];
          }
        } catch (e) {
          console.warn('[Store] Failed to restore tab state, using defaults:', e);
          selectedFile = canvas.files.length > 0 ? canvas.files[0] : null;
          openTabs = selectedFile ? [selectedFile] : [];
        }
      ...
      console.error('[Store] Failed to load canvas:', error);
    }
  },
  
  createCanvasFromSession: (canvasData) => {
    console.log('[Store] Creating canvas from session:', canvasData);
    // Ensure all required arrays are initialized
    const canvas: CodeCanvas = {
      ...canvasData,
      files: canvasData.files || [],
      tasks: canvasData.tasks || [],
      collaborations: canvasData.collaborations || []
    };
  ...
  updateFileContent: async (canvasId, fileId, content) => {
    try {
      console.log(`[Store] Updating file ${fileId} in canvas ${canvasId}...`);
      const response = await fetch(`http://localhost:3001/api/canvas/${canvasId}/files/${fileId}`, {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        console.log(`[Store] File updated successfully`);
        ...
        set((state) => {
          ...
          return {
            canvasState: {
              ...
              selectedFile: state.canvasState.selectedFile?.id === fileId 
                ? updatedFile 
                : state.canvasState.selectedFile
            }
          };
        });
      }
    } catch (error) {
      console.error('[Store] Failed to update file:', error);
      throw error;
    }
  },
  
  renameCanvas: async (canvasId: number, title: string) => {
    try {
      console.log(`[Store] Renaming canvas ${canvasId} to "${title}"...`);
      const response = await fetch(`http://localhost:3001/api/canvas/${canvasId}/rename`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        console.log(`[Store] Canvas renamed successfully`);
        
        // Update local state
        set((state) => {
          const updatedList = state.canvasState.canvasList.map(c =>
          ...
          return {
            canvasState: {
              ...state.canvasState,
              activeCanvas: state.canvasState.activeCanvas?.id === canvasId
                ? { ...state.canvasState.activeCanvas, title }
                : state.canvasState.activeCanvas,
      ...
      console.error('[Store] Failed to rename canvas:', error);
      throw error;
    }
  },
  
  // Tab management actions
  openFileInTab: (file) => set((state) => {
    // Check if file is already open
    const isAlreadyOpen = state.canvasState.openTabs.some(tab => tab.id === file.id);
    if (isAlreadyOpen) {
      // Just select it
      const newState = {
        canvasState: {
      ...
      // Persist selected file ID
      if (state.canvasState.activeCanvas) {
        localStorage.setItem(`canvas-${state.canvasState.activeCanvas.id}-selected-file`, file.id.toString());
      }
      return newState;
    }
    
    // Add to tabs and select
    const newTabs = [...state.canvasState.openTabs, file];
    const newState = {
      canvasState: {
        ...state.canvasState,
        openTabs: newTabs,
    ...
    // Persist tabs and selected file
    if (state.canvasState.activeCanvas) {
      localStorage.setItem(`canvas-${state.canvasState.activeCanvas.id}-open-tabs`, JSON.stringify(newTabs.map(t => t.id)));
      localStorage.setItem(`canvas-${state.canvasState.activeCanvas.id}-selected-file`, file.id.toString());
    }
  ...
  closeTab: (fileId) => set((state) => {
    const newTabs = state.canvasState.openTabs.filter(tab => tab.id !== fileId);
    let newSelectedFile = state.canvasState.selectedFile;
    
    // If closing the selected file, select another one
    if (state.canvasState.selectedFile?.id === fileId) {
      if (newTabs.length > 0) {
        // Select the tab to the right, or the last tab if closing the last one
        const closedIndex = state.canvasState.openTabs.findIndex(tab => tab.id === fileId);
        newSelectedFile = newTabs[Math.min(closedIndex, newTabs.length - 1)];
      } else {
    ...
    if (state.canvasState.activeCanvas) {
      localStorage.setItem(`canvas-${state.canvasState.activeCanvas.id}-open-tabs`, JSON.stringify(newTabs.map(t => t.id)));
      if (newSelectedFile) {
        localStorage.setItem(`canvas-${state.canvasState.activeCanvas.id}-selected-file`, newSelectedFile.id.toString());
      } else {
        localStorage.removeItem(`canvas-${state.canvasState.activeCanvas.id}-selected-file`);
      }
    }
  ...
  // Diff approval workflow actions
  setDiffApprovalRequest: (request) => set((state) => ({
    canvasState: {
      ...state.canvasState,
      pendingDiffApproval: request
    }
  })),
  
  clearDiffApprovalRequest: () => set((state) => ({
    canvasState: {
      ...state.canvasState,
      pendingDiffApproval: null
    }
  ...
  updateFileStatus: (fileId, status) => set((state) => {
    const newStatuses = new Map(state.canvasState.fileStatuses);
  ...
  clearFileStatus: (fileId) => set((state) => {
    const newStatuses = new Map(state.canvasState.fileStatuses);
  ...
  clearAllFileStatuses: () => set((state) => ({
    canvasState: {
      ...state.canvasState,
      fileStatuses: new Map()
    }
  ...
  // Phase 14: New state management actions
  updateOperationMetrics: (metrics: Partial<OperationMetrics>) => set((state) => ({
    canvasState: {
      ...state.canvasState,
      operationMetrics: {
        ...state.canvasState.operationMetrics,
        ...metrics
      }
    }
  })),
  
  addErrorRecoveryAttempt: (subtaskId: number, success: boolean) => set((state) => {
    const newAttempts = new Map(state.canvasState.errorRecoveryAttempts);
    const existing = newAttempts.get(subtaskId);
    
    if (existing) {
      const newAttemptCount = existing.attemptCount + 1;
      const newSuccessCount = success ? 
        Math.round(existing.successRate * existing.attemptCount) + 1 : 
        Math.round(existing.successRate * existing.attemptCount);
      
      newAttempts.set(subtaskId, {
  ...
  addActiveOperation: (operation: ActiveOperation) => set((state) => ({
    canvasState: {
      ...state.canvasState,
      activeOperations: [...state.canvasState.activeOperations, operation]
    }
  })),
  
  updateActiveOperation: (operationId: string, updates: Partial<ActiveOperation>) => set((state) => ({
    canvasState: {
      ...state.canvasState,
      activeOperations: state.canvasState.activeOperations.map(op =>
        op.id === operationId ? { ...op, ...updates } : op
      )
    }
  })),
  
  removeActiveOperation: (operationId: string) => set((state) => ({
    canvasState: {
      ...state.canvasState,
      activeOperations: state.canvasState.activeOperations.filter(op => op.id !== operationId)
    }
  })),
  
  addOperationHistoryEntry: (entry: OperationHistoryEntry) => set((state) => {
    const newHistory = [entry, ...state.canvasState.operationHistory];
    // Keep last 200 entries
    if (newHistory.length > 200) {
      newHistory.splice(200);
    }
  ...
  updateGenerationStatistics: (updates: Partial<GenerationStatistics>) => set((state) => ({
    canvasState: {
      ...state.canvasState,
      generationStatistics: {
        ...state.canvasState.generationStatistics,
        ...updates,
        lastSessionDate: new Date().toISOString()
      }
    }
  ...
  updateSubtaskProgress: (subtaskId: number, updates: {
    generationPhase?: 'idle' | 'planning' | 'generating' | 'applying' | 'complete' | 'failed';
    ...
    const updatedTasks = state.canvasState.activeCanvas.tasks.map(task => ({
      ...task,
